...
func _build_model() -> void:
	...
	for side in [-1, 1]:
		...
		nac.mesh = nm; nac.rotation_degrees = Vector3(90, 0, 0)
		nac.position = Vector3(side * 2.5, 0, -3.5); add_child(nac)
		...
		glow.material_override = gmat; glow.position = Vector3(side * 2.5, 0.0, -6.0)
		add_child(glow)
