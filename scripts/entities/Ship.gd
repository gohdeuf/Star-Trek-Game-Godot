class_name Ship
extends Node3D
# Spielerschiff "Enterprise" (siehe Referenz Abschnitt 6).
#
# Einfache eigene Flug-Physik statt CharacterBody3D, da im Weltraum keine
# Kollisions-/Gravitations-Physik benoetigt wird (Empfehlung aus der Referenz).
# Modell wird aus Primitiven gebaut (Saucer, Hull, Nacelles, Deflector).
# Spaeter austauschbar gegen ein .glb-Modell (siehe _build_model()).

const DEFAULT_SPEED := 150.0
const DEFAULT_ROTATION_SPEED := 100.0  # Grad/Sekunde
...
func _handle_rotation(delta: float) -> void:
	var rot_amount := deg_to_rad(rotation_speed_deg * delta)
	if Input.is_action_pressed("pitch_up"):
		rotate_object_local(Vector3.RIGHT, rot_amount)
	if Input.is_action_pressed("pitch_down"):
		rotate_object_local(Vector3.RIGHT, -rot_amount)
	if Input.is_action_pressed("yaw_left"):
		rotate_object_local(Vector3.UP, rot_amount)
	if Input.is_action_pressed("yaw_right"):
		rotate_object_local(Vector3.UP, -rot_amount)
	if Input.is_action_pressed("roll_left"):
		rotate_object_local(Vector3.FORWARD, rot_amount)
...
func _build_model() -> void:
	...
	# Nacelles (zwei, mit Glow-Spheren)
	for side in [-1, 1]:
