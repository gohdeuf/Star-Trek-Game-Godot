...
# Neu: antimatter im Inventar, is_docked / docked_sector_id / docked_position.
...
func _generate_random_seed() -> int:
	randomize()
	var high: int = randi(); var low: int = randi()
	return (high << 32) | low
...
func save_world_meta() -> void:
	...
	if f == null:
		push_error("GameDatabase: world_meta.json kann nicht geschrieben werden."); return
...
func save_sector_data(sector_id: String, data: Dictionary) -> void:
	...
	if f == null:
		push_error("GameDatabase: Sektordatei kann nicht geschrieben werden: %s" % sector_id); return
	f.store_string(JSON.stringify(data, "  ")); f.close()
...
func save_planet_state(sector_id: String, planet_name: String, minerals: float, deuterium: float) -> void:
	...
	data["planet_resources"] = min_ov; data["planet_deuterium"] = deu_ov
