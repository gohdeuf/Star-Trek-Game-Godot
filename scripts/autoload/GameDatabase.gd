...
var station_storage:  Dictionary = {}  # station_id -> {minerals, deuterium, antimatter}
...
func deposit_to_station(sid: String, cap_min: int, cap_deu: int, cap_am: int) -> void:
	...
	# Uebertrage alles was reinpasst
...
func withdraw_from_station(sid: String) -> void:
	var st := get_station_storage(sid)
	var w_min: int = int(st.get("minerals",  0.0))
	var w_deu: int = int(st.get("deuterium", 0.0))
	var w_am:  int = int(st.get("antimatter",0.0))
	add_resource("minerals",  float(w_min))
	add_resource("deuterium", float(w_deu))
	add_resource("antimatter",float(w_am))
