...
func _generate_sector(sector_id: String) -> Dictionary:
	...
	# Explizite Typen bei SECTOR_UTILS-Aufrufen vermeiden Variant-Inferenzfehler
...
func _generate_moons(rng: RandomNumberGenerator, cls: String, planet_radius: float, planet_name: String) -> Array:
	...
	var moons: Array = []
	var orbit_radius: float = planet_radius + rng.randf_range(4.0, 8.0)
...
func _build_sol_system() -> Dictionary:
	...
	var angle_step: float = 360.0 / planet_defs.size()
	var planets: Array = []
	for i in range(planet_defs.size()):
		...
		var moons: Array = []
		var moon_orbit: float = planet_radius + 4.0
		for md in def["moons"]:
			moon_orbit += 4.0
			moons.append({"name":"%s %s"%[def["name"],md["suffix"]],"orbit_radius":moon_orbit,"angular_speed_deg":md["speed"]})
		planets.append({
			"name":def["name"],"class":cls,"orbit_radius":def["orbit_radius"],"orbit_angle":angle_step*i,
...
func _apply_resource_overrides(sector_id: String, system: Dictionary) -> void:
	...
	if min_ov.is_empty() and deu_ov.is_empty():
		return
