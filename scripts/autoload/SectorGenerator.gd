...
func _generate_sector(sector_id: String) -> Dictionary:
	...
	var star_pos := origin + Vector3(rng.randf_range(0.0,SECTOR_UTILS.SECTOR_SIZE),rng.randf_range(0.0,SECTOR_UTILS.SECTOR_SIZE),rng.randf_range(0.0,SECTOR_UTILS.SECTOR_SIZE))
	...
	for i in range(planet_count):
		...
		planets.append({"name":planet_name,"class":cls,"orbit_radius":orbit_radius,"orbit_angle":orbit_angle,
			"radius":planet_radius,"resources":resources,"deuterium":deuterium,
			"moons":_generate_moons(rng, cls, planet_radius, planet_name)})
	return {"system_id":sector_id+"_sys","sector_id":sector_id,"name":star_name,
...
func _generate_moons(rng: RandomNumberGenerator, cls: String, planet_radius: float, planet_name: String) -> Array:
	...
	for i in range(max_moons):
		...
		moons.append({"name":"%s %s"%[planet_name,_moon_letter(moons.size())],
			"orbit_radius":orbit_radius,"angular_speed_deg":rng.randf_range(10.0,40.0)})
	return moons
...
func _build_sol_system() -> Dictionary:
	...
	var angle_step: float = 360.0 / planet_defs.size(); var planets: Array = []
	for i in range(planet_defs.size()):
		...
		var res_max: float = (float(res_r[0]) + float(res_r[1])) * 0.5
		var deu_r: Array = PlanetClassDB.classes[cls]["deuterium"]
		var deu_max: float = (float(deu_r[0]) + float(deu_r[1])) * 0.5
		...
		planets.append({"name":def["name"],"class":cls,"orbit_radius":def["orbit_radius"],"orbit_angle":angle_step*i,
			"radius":planet_radius,"resources":{"max":res_max,"current":res_max},
			"deuterium":{"max":deu_max,"current":deu_max},"moons":moons})
	return {"system_id":SOL_SECTOR_ID+"_sys","sector_id":SOL_SECTOR_ID,"name":"Sol",
