...
func weighted_random_class(rng: RandomNumberGenerator) -> String:
	var total_weight := 0
	for key in classes.keys(): total_weight += int(classes[key]["weight"])
	var roll := rng.randi_range(1, total_weight); var cumulative := 0
	for key in classes.keys():
		cumulative += int(classes[key]["weight"])
		if roll <= cumulative: return key
	return "M"
func random_resources(rng: RandomNumberGenerator, cls: String) -> Dictionary:
	var range_vals: Array = classes[cls]["resources"]; var max_val: int
	if int(range_vals[1]) > int(range_vals[0]): max_val = rng.randi_range(int(range_vals[0]), int(range_vals[1]))
	else: max_val = int(range_vals[0])
	return {"max": max_val, "current": max_val}
func random_deuterium(rng: RandomNumberGenerator, cls: String) -> Dictionary:
	var range_vals: Array = classes[cls]["deuterium"]; var max_val: int
	if int(range_vals[1]) > int(range_vals[0]): max_val = rng.randi_range(int(range_vals[0]), int(range_vals[1]))
	else: max_val = int(range_vals[0])
	return {"max": max_val, "current": max_val}
